...
class gp_interpolator:
    ...
    def read_rainfall(self, obs, sim, mu_x = None, kxx = None):
    ...
    def sn_converge(self):
        ...
        while np.mean((sn0 - self.sn)**2) / np.mean(self.sn**2) > self.sn_thres:
            ...
            for j in range(self.p_):
                # Create mask to exclude station j
                mask = np.ones(self.p_, dtype=bool)
                mask[j] = False
                
                # Extract masked variables directly from stored values
                tsn = self.sn[mask]
                tmu_x = self.mu_x[mask]
                tkxx = self.kxx[np.ix_(mask, mask)]
                tkyx = self.kxx[j:j+1, mask]
                tmu_y = self.mu_x[j]
                tkyy = self.kxx[j, j]
                txx_obs = xx_obs[:, mask]
    ...
    def predict(self, yy, mu_y = None, kyy = None, kyx = None):
        if mu_y is None:
